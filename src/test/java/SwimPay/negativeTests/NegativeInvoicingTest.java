...
public class NegativeInvoicingTest extends baseClass {
	...
	// Verify if user is able to upload the pdf extension Document in the
	// Instruction submenu
	@Test(priority = 1)
	...
	// Verify if user is able to upload the doc extension Document in the
	// Instruction submenu
	@Test(priority = 2)
	...
	// Verify if user is able to upload the csv extension Document in the
	// Instruction submenu
	@Test(priority = 3)
	...
	// Verify if user is able to upload the oversize logo and can get the proper
	...
	// Verify if user is able to create an invoice without inserting any details in
...
//		invoicingPage.SelectNextMonth("July 2024");
	...
	// Verify if user is able to create an invoice for Swim-Pay Customer with
	// invalid customer/account name
	@Test(priority = 7)
...
//		invoicingPage.SelectNextMonth("July 2024");
		...
		Assert.assertTrue(invoicingPage.DisplayButtons(12)); // Customer btn
		LoginPage.waitcode();
		try {
	...
	// Verify if user is able to add the items and insert the invalid Item
	// description Cost & Quantity details in Create Invoice Sub-menu
	@Test(priority = 8)
...
//		invoicingPage.SelectNextMonth("July 2024");
	...
	// Verify if user is able to insert the invalid details in Discount & tax field
	// and able to create the invoice
	@Test(priority = 9)
...
//		invoicingPage.SelectNextMonth("July 2024");
	...
	// Verify if user is able to search by inserting invalid Reference and date
	// details in invoice list sub-menu
	@Test(priority = 10)
	public void InsertInvalidDetailsAndVerifySearchBtnIsEnabled() {
		loginPage LoginPage = new loginPage(driver);
		LoginPage.userEmail(email1);
		LoginPage.loginEmailSubmit();
		LoginPage.userPassword(password);
		...
		invoicingPage.waitcode();
		Assert.assertFalse(invoicingPage.DisplaySearchBtn());

	}

	// Verify if user is able to enter after date then date in To field and click on
	// search button in invoice list submenu
	@Test(priority = 11)
	...
	// on search button in invoice list submenu
	@Test(priority = 12)
